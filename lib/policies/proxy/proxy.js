...
module.exports = function (params, config) {
  ...
  if (intermediateProxyUrl) {
    console.log(intermediateProxyUrl);
    logger.verbose(`using intermediate proxy ${intermediateProxyUrl}`);
    const extensionOrTLD = intermediateProxyUrl.split('.');
    if (extensionOrTLD[extensionOrTLD.length - 1] === 'pac') {
      proxyOptions.agent = new PacProxyAgent(intermediateProxyUrl);
    } else {
